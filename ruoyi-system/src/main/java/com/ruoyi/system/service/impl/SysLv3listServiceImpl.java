package com.ruoyi.system.service.impl;

import java.util.List;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import com.ruoyi.system.mapper.SysLv3listMapper;
import com.ruoyi.system.domain.SysLv3list;
import com.ruoyi.system.service.ISysLv3listService;

/**
 * 三级列表Service业务层处理
 * 
 * @author ruoyi
 * @date 2021-06-05
 */
@Service
public class SysLv3listServiceImpl implements ISysLv3listService 
{
    @Autowired
    private SysLv3listMapper sysLv3listMapper;

    /**
     * 查询三级列表
     * 
     * @param id 三级列表ID
     * @return 三级列表
     */
    @Override
    public SysLv3list selectSysLv3listById(Long id)
    {
        return sysLv3listMapper.selectSysLv3listById(id);
    }

    /**
     * 查询三级列表列表
     * 
     * @param sysLv3list 三级列表
     * @return 三级列表
     */
    @Override
    public List<SysLv3list> selectSysLv3listList(SysLv3list sysLv3list)
    {
        return sysLv3listMapper.selectSysLv3listList(sysLv3list);
    }

    /**
     * 新增三级列表
     * 
     * @param sysLv3list 三级列表
     * @return 结果
     */
    @Override
    public int insertSysLv3list(SysLv3list sysLv3list)
    {
        return sysLv3listMapper.insertSysLv3list(sysLv3list);
    }

    /**
     * 修改三级列表
     * 
     * @param sysLv3list 三级列表
     * @return 结果
     */
    @Override
    public int updateSysLv3list(SysLv3list sysLv3list)
    {
        return sysLv3listMapper.updateSysLv3list(sysLv3list);
    }

    /**
     * 批量删除三级列表
     * 
     * @param ids 需要删除的三级列表ID
     * @return 结果
     */
    @Override
    public int deleteSysLv3listByIds(Long[] ids)
    {
        return sysLv3listMapper.deleteSysLv3listByIds(ids);
    }

    /**
     * 删除三级列表信息
     * 
     * @param id 三级列表ID
     * @return 结果
     */
    @Override
    public int deleteSysLv3listById(Long id)
    {
        return sysLv3listMapper.deleteSysLv3listById(id);
    }
}
